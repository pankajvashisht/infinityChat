...
import { editCategory as EditExitsCategory } from 'Apis/admin';
import Category from 'containers/forms/Category';
import { NotificationManager } from 'components/common/react-notifications';
const EditCategory = React.memo(({ history, location }) => {
	...
	const updateCategory = (event) => {
		...
		setIsLoading(true);
		EditExitsCategory(categoryForm)
			.then(() => {
	...
	return (
		<Fragment>
			<Row>
				<Colxx xxs='12'>
					<h1>Edit Category ({categoryForm.name})</h1>
