...
module.exports = {
	...
	categoryDetails: async (Request) => {
		...
		const goals = await DB.first(
			`select goals.*, (select count(id) from user_goals where user_id=${user_id} and goal_id =goals.id) as is_added,categories.name as category_name  from goals join categories on (goals.category_id = categories.id)  where category_id=${category_id} order by id desc`
		);
	...
	getProgress: async (Request) => {
		...
		const query = `select goals.*, user_goals.id as user_goal_id, (select count(id) from goal_progresses where user_id = ${user_id} and user_goal_id = user_goals.id  and from_unixtime(date, '%Y%D%M') = from_unixtime(${timeStamp}, '%Y%D%M')) as is_done, 1 as is_added   from user_goals join goals on (goals.id = user_goals.goal_id) where user_id=${user_id}  order by user_goal_id desc limit ${offset}, ${limit}`;
		...
		const completeGoal = await DB.first(
			`select count(goal_progresses.id) as total from goal_progresses join user_goals on (goal_progresses.user_goal_id = user_goals.id ) where goal_progresses.user_id = ${user_id} and from_unixtime(date, '%Y%D%M') = from_unixtime(${timeStamp}, '%Y%D%M')`
		);
		...
		const progress = {
			...
			totalProgress: Math.round((completedGoal / totalRecord) * 100, 1) || 0,
		};
