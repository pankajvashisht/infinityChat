...
module.exports = {
	...
	getProgress: async (Request) => {
		...
		const completeGoal = await DB.first(
			`select count(id) as total from goal_progresses join user_goals on (goal_progresses.user_goal_id = user_goals.id ) where user_id = ${user_id} and from_unixtime(date, '%Y%D%M') = from_unixtime(${timeStamp}, '%Y%D%M')`
		);
